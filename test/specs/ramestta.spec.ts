...
import {ABIManager, Converter, setProofApi} from "@ramestta/ramajs";


describe('RAMESTTA', () => {

    let erc20Child = plasmaClient.erc20(null);
    let erc20Parent = plasmaClient.erc20(erc20.rama, true);
    ...
    it('withdrawstart return tx', async () => {
        ...
        console.log('result', result)
        expect(result['to'].toLowerCase()).equal(erc20.ramaChild.toLowerCase());
    ...
    it('approve return tx', async () => {
        ...
        expect(result['to'].toLowerCase()).equal(erc20.rama.toLowerCase());
        expect(result).to.have.property('data')
