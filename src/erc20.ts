import { TYPE_AMOUNT, Converter, Web3SideChainClient, ITransactionOption, MAX_AMOUNT, IApproveTransactionOption, ERROR_TYPE, IAllowanceTransactionOption } from "@ramestta/ramajs";
import { RAMA_TOKEN_ADDRESS_ON_RAMESTTA } from "./constant";
...
export class ERC20 extends PlasmaToken {
    ...
    constructor(
        ...
        const isRamaToken = tokenAddress === RAMA_TOKEN_ADDRESS_ON_RAMESTTA;
        const contractIdentifier = isRamaToken ?
            'MRC20' : 'ChildERC20';
        super({
            isParent,
    ...
    withdrawStart(amount: TYPE_AMOUNT, option: ITransactionOption = {}) {
        ...
        if (this.contractParam.address === RAMA_TOKEN_ADDRESS_ON_RAMESTTA) {
            option.value = Converter.toHex(amount);
        }
